...
class SFBrowserSession {
    ...
    async fetch(entityPath, params) {
        ...
        if (result.status === 401) {
            this.loggedIn = false;
            await this.login();
            const retry = await p.evaluate(async ([fetchUrl]) => {
                const r = await fetch(fetchUrl, {
                    credentials: 'include',
                    headers: { Accept: 'application/json' },
                });
...
server.tool('authenticate', 'Log in to SAP SuccessFactors via Playwright and cache the session cookie.', {}, async () => {
    try {
        ...
        if (status === 200)
            return { content: [{ type: 'text', text: 'Authentication successful (Browser SSO).' }] };
...
// Tool: run_model
server.tool('run_model', 'Run the Python attrition risk model and rebuild the HTML dashboard.', { dashboard_only: z.boolean().optional().default(false) }, async ({ dashboard_only }) => {
...
server.tool('get_high_risk_employees', 'Return the top N high-risk employees from the last model run.', { top_n: z.number().optional().default(20) }, async ({ top_n }) => {
