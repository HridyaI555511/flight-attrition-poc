...
async function odataFetch(entityPath: string, params: Record<string, string> = {}): Promise<{ status: number; data: unknown; raw: string }> {
...
server.tool(
  ...
  async ({ entities }) => {
    const DEFAULT_ENTITIES: Array<{ entity: string; name: string; top?: number; select?: string; filter?: string }> = [
